...
if __name__ == '__main__':
    ...
    all_bv_webblat_df.loc[all_bv_webblat_df['bim_mapped_rsID']=='JHU_3.16652239', 'bim_mapped_rsID'] =  'rs9870157'
    all_bv_webblat_df.loc[all_bv_webblat_df['bim_mapped_rsID']=='JHU_13.20119335', 'bim_mapped_rsID'] = 'rs9508454'
    all_bv_webblat_df.loc[all_bv_webblat_df['bim_mapped_rsID']=='14:35761675-C-G', 'bim_mapped_rsID'] = 'rs1048990'

    # %ll
    # plot the all blat hit
    sns.set(style="whitegrid", font_scale=1.5)
    for label, df in zip(['UKBB','BioVU'], [all_ukwcsf_webblat_df, all_bv_webblat_df]):
        fig, axs = plt.subplots(nrows=1, ncols=1, figsize=(8,6))


        temp_df = pd.concat([df.loc[df['stat_sig']==True], df.loc[df['stat_sig']==False]], axis=0)
        temp_df.loc[temp_df['stat_sig']==False, "per_identity"] = np.nan
        temp_df.loc[temp_df['stat_sig']==False, "bim_mapped_rsID"] = np.nan
        temp_df['length_bin'] = np.ceil(temp_df['q.length']/10)


        sns.violinplot(data=df, x="stat_sig", y="per_identity", color="white", inner='quartile', ax=axs, cut=0, width=0.8, scale='width')
        sns.stripplot(data=temp_df, x='stat_sig', y='per_identity',  edgecolor="black", ax=axs, hue="bim_mapped_rsID", palette=sns.color_palette("Set1", n_colors=8, desat=.5),
                                   size=5, alpha=0.9, jitter=0.05, dodge=True, marker='o', rasterized=False, linewidth=0.4)


        n_no_signif = df.loc[df['stat_sig']==False, 'bim_mapped_rsID'].nunique()
        n_signif = df.loc[df['stat_sig']==True, 'bim_mapped_rsID'].nunique()



        axs.set_xlabel("Variant GWAS P-value", fontproperties=prop, labelpad=20)
        axs.set_ylabel("Probe sequence identity (%)", fontproperties=prop)
        axs.set_title("{} probes with X or Y BLAT hit".format(label),  fontproperties=prop)
        axs.set_xticklabels(["Non-significant\n(n={:,})".format(n_no_signif), "Significant\n(n={:,})".format(n_signif)], fontproperties=prop)
        axs.legend(title="",loc='center left', bbox_to_anchor=(1, 0.5))
        patch_violinplot(['black'])


        lines = [art for art in axs.get_children() if isinstance(art, Line2D)]

        for line in lines:
            line.set_linestyle('-')
            line.__setattr__('_color', 'indianred')
            line.__setattr__('_linewidth', 2.5)


        plt.tight_layout()
        plt.savefig(os.path.join(OUTPUT_DIR, '{}_best_blat_perID_{}.pdf'.format(label,DATE)))
        plt.savefig(os.path.join(OUTPUT_DIR, '{}_best_blat_perID_{}.png'.format(label,DATE)))
