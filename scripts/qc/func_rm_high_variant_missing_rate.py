...
@track_fx
def rm_high_variant_missing_rate(input_prefix, output_dir, base_prefix):
    """
    Uses plink to remove variants with high missing rate.
    ...
    Returns
    -------
    vars_miss_removed_plink_prefix : str
        - full path with plink prefix removing variatns with high missing rate 
    ...
    # =============  OUTPUT FILES =============
    vars_miss_removed_plink_prefix = os.path.join(output_dir, "temp_variants_missing_{}".format(base_prefix))
    ...
    shell_cmd = ("plink --bfile {}"
                ...
                " --snps-only"
                " --make-bed"
                " --out {}").format(input_prefix, vars_miss_removed_plink_prefix)

    rm_miss_var_stdout = run_shell_cmd(shell_cmd)


    return vars_miss_removed_plink_prefix, rm_miss_var_stdout