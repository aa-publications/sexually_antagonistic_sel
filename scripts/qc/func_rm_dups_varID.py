...
def rm_dup_var_id(input_prefix, output_dir, base_prefix):
    ...
    id_dup_vars_cmd = "plink --bfile {} --list-duplicate-vars suppress-first ids-only  --out {}".format(
        input_prefix,  dup_vars_output)
    ...
    if os.stat(dup_vars_output+".dupvar").st_size != 0:
        rm_dups_cmd = "plink --bfile {} --exclude {} --make-bed --out {}".format(
            input_prefix, dup_vars_output+".dupvar", no_dups_vars_plink_prefix)
    else:
