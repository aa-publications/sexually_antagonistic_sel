...
# modified on: 2019-07-03 08:58:06

# NOTE:
# In func_rm_discordant_sex.py:
...
sys.path.append("/dors/capra_lab/users/abraha1/prelim_studies/katja_biobank/scripts/MEGAex_BioVUthru2019-02")
from func_extract_race_white import extract_race_white
...
# plink_prefix = "MEGAex_BioVUthru2019-02_BestOfMultipleCalls_Capra_Preterm_A3"  # prefix before .bed/.bim/.ped
...
# output_dir = "/dors/capra_lab/users/abraha1/prelim_studies/katja_biobank/temp"
# !!!!! ##### !!!!! #### !!!!! ##### !!!!! #### !!!!! ##### !!!!! ##### !!!!! ##### !!!!!
...
# =============  SET UP =============
print("Running {} on {}...\
        \n\tfile: {}\n\tdata_dir: {}\n\toutput_dir: {}\n\n".format(os.path.basename(__file__), DATE, plink_prefix, data_dir, output_dir))

raw_plink_prefix = os.path.join(data_dir, plink_prefix)
base_prefix = plink_prefix

# track num of snps and individuals through each qc step
...
update_counts('rm_cctrl_miss_snps', fam_ct, bim_ct)

#
#   KEEP ONLY WHITE INDIVIDUALS INDIVIDUALS
#

(ceu_only_plink_prefix, plink_stdout), fam_ct, bim_ct = extract_race_white(
    vars_miss_removed_plink_prefix, output_dir, base_prefix, prefix='temp_white_only')
update_counts('keep_white', fam_ct, bim_ct)


# =============  CALC FINAL STATS =============
(frq_and_miss_plink_prefix, plink_stdout), fam_ct, bim_ct = calc_stats(
    ceu_only_plink_prefix, output_dir, base_prefix, prefix='final_qc')
